...
```bash
wget https://github.com/amadvance/snapraid-daemon/releases/download/v1.9/snapraid-daemon-1.9.tar.gz
tar xzvf snapraid-daemon-1.9.tar.gz
cd snapraid-daemon-1.9/
./configure
make -j"$(nproc)"
make check
make install
cd ..
rm -rf snapraid-daemon-1.9*
