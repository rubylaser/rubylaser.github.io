...
```bash
wget https://github.com/amadvance/snapraid-daemon/releases/download/v1.8/snapraid-daemon-1.8.tar.gz
tar xzvf snapraid-daemon-1.8.tar.gz
cd snapraid-daemon-1.8/
./configure
make -j"$(nproc)"
make check
make install
cd ..
rm -rf snapraid-daemon-1.8*
