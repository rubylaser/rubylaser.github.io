---
layout: post
title: "SnapRAID 14.X and the New SnapRAID Daemon: A First Look"
date: 2026-03-30
...
SnapRAID 14.X is a significant release, and the biggest reason it exists is to lay the groundwork for the new daemon — but it also brings some genuinely useful CLI improvements on its own.
...
- **New `locate` command** — Maps physical file offsets within the parity volume. 
  Mostly a diagnostic tool, but handy if you're ever doing deep analysis of parity   distribution.
...
```bash
wget https://github.com/amadvance/snapraid/releases/download/v14.6/snapraid-14.6.tar.gz
tar xzvf snapraid-14.6.tar.gz
cd snapraid-14.6
./configure
make -j"$(nproc)"
make check
...
```bash
wget https://github.com/amadvance/snapraid-daemon/releases/download/v1.11/snapraid-daemon-1.11.tar.gz
tar xzvf snapraid-daemon-1.11.tar.gz
cd snapraid-daemon-1.11/
./configure
make -j"$(nproc)"
make check
make install
cd ..
rm -rf snapraid-daemon-1.11*
...
Since you likely have 2FA enabled on your Google account, you'll need an app password rather than your regular password. Head to [myaccount.google.com/apppasswords](https://myaccount.google.com/apppasswords), create one (I called mine "snapraidd"), and save the 16-character password.
...
Lock down the permissions or curl will refuse to use it if it's wrong!
```bash
...
**NOTE: Finally, make sure you change it to run as root instead of as nobody.**
...
The daemon's hook system makes this straightforward. It calls a script at three points in the task lifecycle:
...
SnapRAID 14.0 + the daemon is a meaningful step forward. The CLI improvements are solid on their own, but the daemon is what makes this feel like a modern storage tool rather than a collection of shell scripts held together with cron. The Safety Freeze alone is worth it if you make sweeping changes. 
