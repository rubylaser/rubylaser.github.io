---
title: 'SnapRAID Split Parity Sync Script'
date: '2016-11-17T16:37:02-05:00'
layout: post
permalink: /snapraid-split-parity-sync-script/
image: /wp-content/uploads/2016/11/split-parity.png
categories: [script, snapraid]
---

[SnapRAID](https://github.com/amadvance/snapraid/blob/master/HISTORY) added a sweet feature, split parity. In the past, adding larger data disk always came with the issue of needing parity disks as large or larger than your data disks. For example, let’s say you add an array made up of (4) 4TB data disks and (1) 4TB parity disk. What if you want to buy one of those 6 or 8TB disks to use as a data disk in your array? In the past, you could have either chosen to use the new larger disk as your new parity disk, or risk having part of your new disk not protected. With split parity, you could use the new 8TB disk as a data disk and then use (2) of your old 4TB disks, joined together as one complete set of parity (or, you could create parity in this scenario with (4) 2TB disks or even (8) 1TB disks). Pretty neat!

So, this would allow you going forward to add 6 or 8TB data disks and have all your data protected, without having to buy an extra one or two larger disks just to use on parity. Now that we’ve discussed split parity, how can we automate syncing like we did with my [previous script](/updated-snapraid-sync-script/)? We can’t use that script as is because of the split parity files. I thought I’d extend it for split parity and add a couple of extra functions. I present you now with the new split parity script (this version is setup for dual parity with 4 disks setup to complete split parity).
